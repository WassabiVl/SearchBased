...
def optimize(population):
    ...
    while iterations < max_iterations:
        ...
        for i in range(int(pop_size / 2)):
            # keep in mind - the functions are also able to get more than one individual this is why we index here with 0
            ### select
...
def select(population, count=2):
    ...
    parents = selectParentSUS(population, fitness_array, count)
    return parents


def selectParentSUS(population, fitness_array, count):
    individual_indices = []
...
def selectParentFPS(population, fitness_array, count):
    individuals = []
    for _ in range(count):
